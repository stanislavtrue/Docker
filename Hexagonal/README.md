...
---
### IOrderRepository.cs
> Описує контракт для збереження та читання замовлень, відокремлює домен від конкретної реалізації БД, та дозволяє легко змінювати інфраструктуру (Postgres, Csv, тощо)
```csharp
